...
## Bounce detection (this session)

Added a way to tell the difference between "the send failed" (already caught) and "the send succeeded but the email actually bounced" (previously invisible).

**Why this needed new code, not just a report:** `GmailApp.sendEmail()` only throws for a *send-time* failure (bad format, empty address, quota) — that's what `sendEmails()` already catches and logs as `Failed: ...`. A true bounce (mailbox doesn't exist, is full, or blocks the message) happens *after* Gmail has already accepted the message for delivery, so there's no API call that tells Apps Script "that one bounced." The only way to find out is to look for the delivery-failure notification email Gmail sends back to the sender's own inbox, which can take anywhere from minutes to hours to arrive.

- New backend: `checkForBounces()` searches the inbox for delivery-failure notifications (`from:mailer-daemon`, `Delivery Status Notification (Failure)`, `Undelivered Mail Returned to Sender`, last 90 days), extracts every email address mentioned in each one, and keeps the first address that matches something we've actually sent to (from the `SendLog` sheet) — this avoids false-matching on unrelated addresses that show up in a bounce message (support links, etc). It also best-effort extracts a short human-readable reason line (`extractBounceReason_()`) from a few common bounce message formats.
- Matches are recorded once each in a new `BounceLog` sheet (created automatically, same settings spreadsheet as `AgentEmails`/`SendLog`) — re-running `checkForBounces()` is safe, already-seen threads (by Gmail thread ID) are skipped so nothing gets logged twice.
- **Status tab**: a new "Bounce Report" card with a **🔍 Check for Bounces** button and a list of everything recorded so far (agent, email, cycle, reason, when it was detected). A bounced agent also gets a red "⚠ Bounced" pill next to their existing "Sent ✓" pill.
- **History tab**: each past cycle now shows a "⚠ N bounced" pill alongside its agent/policy counts, cross-referenced from the same `BounceLog` sheet.
- **Real limitation, not just an edge case**: this can only catch bounces that have already landed in the inbox by the time you click the button, and address/reason extraction is a heuristic over free-text email bodies from whatever mail server rejected the message — not a guaranteed parse of every possible bounce format. It will not catch a "soft" bounce/spam-folder situation where the recipient's server accepts but never delivers without complaint.
- **Permission change**: this required adding the `gmail.readonly` OAuth scope to `appsscript.json` (previously only `gmail.send` was granted) so the script can search and read inbox messages. This means Tanya (or whoever the script runs as) will see a **re-authorization prompt** the next time the script runs after this deploy, asking to grant read access to Gmail in addition to the existing send access.
- Verified with the dry-run harness: a fake bounce thread mentioning MARY BANDA's address is correctly matched to her most recent "Sent" log entry, the reason is extracted, a second run doesn't double-log it, and the match is correctly reflected in `getBounces()`, `getSendHistory()`'s per-cycle `bounced` count, and `getStatusSummary()`'s per-agent `bounced` flag. Also verified with Playwright screenshots of the full Status/History/Bounce Report UI.

## Access code gate (this session)

Added a simple sign-in screen in front of the whole portal, gated by a fixed access code (`2468`, set as `ACCESS_CODE` in `Code.gs`).

- New `Login.html` file: a standalone branded sign-in card (matches the portal's navy/gold styling) with a single access-code field and a plain `<form method="get">` — no client-side JS needed.
- `doGet(e)` now checks `e.parameter.code` before deciding what to serve: the right code renders `Index.html` (the actual portal) exactly as before; anything else — including no code at all — renders `Login.html` instead, showing an "Incorrect access code" message only if a (wrong) code was actually submitted.
- **This is a real, server-side gate, not a cosmetic overlay**: an unauthenticated visitor's browser never receives the portal's HTML/JS at all — only the login page — until the correct code is submitted. That's a meaningfully stronger position than a client-side "unlock" script layered on top of the existing page, which was the other option (that approach ships the whole app to the browser regardless and just hides it with CSS/JS, trivially bypassed via dev tools).
- **Honest limitation**: this is one shared code for everyone, sent as a URL query parameter (`?code=2468`) rather than through a real auth system — it'll show up in browser history and Apps Script's own execution logs. It's a reasonable deterrent against a stranger stumbling onto the URL (relevant given `appsscript.json` still sets `access: "ANYONE_ANONYMOUS"`, flagged earlier in this README), not protection against someone who already has or guesses the link and code. After entering the code once, bookmarking the resulting URL (which will include `?code=2468`) skips the login screen on future visits.
- `Login.html` was added to `.claspignore`'s allowlist alongside `Code.gs`/`Index.html`/`appsscript.json` so `clasp push` actually ships it.
